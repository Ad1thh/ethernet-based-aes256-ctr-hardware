...
module aes256_ctr_core (
    ...
    // The pipeline shifts if it is not stalled, AND we have either new input OR data to flush
    wire shift_en = pipeline_en && (in_valid || pipeline_has_data);

    // Input is ready if the pipeline is not stalled
    assign in_ready = pipeline_en;
