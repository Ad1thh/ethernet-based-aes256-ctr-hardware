...
void send_response(struct tcp_pcb *tpcb) {
    ...
    if (data_sent_bytes == header.file_size) {
        xil_printf("Transaction complete. Sent %u bytes.\r\n", (unsigned int)data_sent_bytes);
...
err_t recv_callback(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err) {
    ...
    while (q != NULL) {
        ...
        uint32_t payload_len = q->len;
        uint8_t *payload_ptr = (uint8_t *)q->payload;
        ...
        while (payload_offset < payload_len) {
            if (session_state == STATE_WAIT_HEADER) {
                ...
                Xil_DCacheInvalidateRange((UINTPTR)(payload_ptr + payload_offset), bytes_to_copy);
                ...
                if (header_received_bytes == HEADER_SIZE) {
                    ...
                    xil_printf("\r\n[PS] Parsed Header - Mode: %d, Size: %u bytes\r\n", header.mode, (unsigned int)header.file_size);
        ...
        if (session_state == STATE_PROCESSING) {
            break;
        }
    ...
    if (session_state == STATE_PROCESSING) {
        xil_printf("[PS] All file data received (%u bytes). Running encryption...\r\n", (unsigned int)data_received_bytes);
                ...
                if (header.mode == 0 || header.mode == 1) {
                    ...
                    uint32_t diff = (uint32_t)(tEnd - tStart);
                    elapsed_us = (uint32_t)(((uint64_t)diff * 1000000) / COUNTS_PER_SECOND);
                    xil_printf("  DEBUG: tStart=%u, tEnd=%u, diff=%u, CPS=%u\r\n", 
                               (unsigned int)tStart, (unsigned int)tEnd, (unsigned int)diff, (unsigned int)COUNTS_PER_SECOND);
                    xil_printf("  Software Complete. Time: %u us\r\n", (unsigned int)elapsed_us);
                } 
                    ...
                    // Byte-swap input data (in place) to match PL stream endianness
                    uint32_t *in_words = (uint32_t *)data_in_buf;
                    uint32_t word_count = header.file_size / 4;
                    uint32_t w;
                    for (w = 0; w < word_count; w++) {
                        in_words[w] = __builtin_bswap32(in_words[w]);
                    }
                    ...
                    // Byte-swap output data back (in place) to match host endianness
                    uint32_t *out_words = (uint32_t *)data_out_buf;
                    for (w = 0; w < word_count; w++) {
                        out_words[w] = __builtin_bswap32(out_words[w]);
                    }
                    ...
                    xil_printf("  DEBUG: tStart=%u, tEnd=%u, diff=%u, CPS=%u\r\n", 
                               (unsigned int)tStart, (unsigned int)tEnd, (unsigned int)diff, (unsigned int)COUNTS_PER_SECOND);
                    xil_printf("  Hardware Complete. Time: %u us\r\n", (unsigned int)elapsed_us);
                }
